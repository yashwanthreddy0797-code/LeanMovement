  ...
  return (
    <div className="space-y-8">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <div className="text-[11px] uppercase tracking-[0.2em] text-[#737373] mb-1.5">Client management</div>
          <h1 className="text-4xl md:text-5xl">All clients <span className="text-[#A3A3A3] text-2xl">· {coachClients.length}</span></h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-[#000000] text-white text-sm font-medium hover:bg-[#111111]">
              ...
              <tr key={c.name} className="border-t border-[var(--border)] hover:bg-[#FAFAF6]">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#F5F5F5] text-[#E11D2A] grid place-items-center text-xs font-semibold">{c.name[0]}</div>
                    <span className="font-medium text-[#000000]">{c.name}</span>
                  </div>
                </td>
                <td className="px-6 py-4 text-[#404040]">{c.program}</td>
                <td className="px-6 py-4 text-[#404040]">{c.goal}</td>
                <td className="px-6 py-4 text-[#404040]">{c.join}</td>
                <td className="px-6 py-4 text-[#404040]">{c.renew}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-medium ${
                    c.status === "Active" ? "bg-[#FEE2E2] text-[#E11D2A]" :
                    c.status === "Onboarding" ? "bg-[#FBF2DC] text-[#A77B2C]" :
                    "bg-[#F6E6DC] text-[#A55A2C]"
                  }`}>{c.status}</span>
                </td>
                <td className="px-6 py-4 text-right text-xs">
                  <button className="text-[#E11D2A] hover:underline mr-3">Open</button>
                  <button className="text-[#737373] hover:underline">Edit</button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 backdrop-blur-sm p-4">
      <div className="card-soft w-full max-w-lg p-7 relative">
        <button onClick={onClose} className="absolute top-4 right-4 w-9 h-9 rounded-full hover:bg-[#F5F5F5] grid place-items-center text-[#737373]">
          <X size={16} />
        </button>
        <div className="text-[11px] uppercase tracking-[0.2em] text-[#737373]">New client</div>
        <h2 className="text-2xl mt-1">Invite a new member</h2>
        <p className="text-sm text-[#737373] mt-1">They'll receive an email with a link to set their password and start onboarding.</p>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onClose(); }} className="mt-6 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <button type="submit" className="ml-auto px-5 py-2.5 rounded-2xl bg-[#000000] text-white text-sm font-medium hover:bg-[#111111]">
              Send invite
            </button>
          </div>
        </form>
