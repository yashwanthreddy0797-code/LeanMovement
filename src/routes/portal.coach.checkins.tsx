...
function CheckIns() {
  return (
    <div className="space-y-8">
      <div>
        <div className="text-[11px] uppercase tracking-[0.2em] text-[#737373] mb-1.5">Check-in management</div>
        <h1 className="text-4xl md:text-5xl">Review & respond</h1>
        <p className="mt-2 text-[#737373]">{coachCheckins.filter(c => c.status === "Pending").length} pending · keep response time under 24h.</p>
      </div>
          ...
          <SoftCard key={i}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-[#000000] text-white grid place-items-center text-sm font-semibold">{c.client[0]}</div>
                <div>
                  <div className="font-medium text-[#000000]">{c.client}</div>
                  <div className="text-[11px] text-[#737373]">{c.date}</div>
                </div>
              </div>
              <span className={`chip ${c.status === "Pending" ? "" : "!bg-[#F5F5F5] !text-[#737373]"}`}>{c.status}</span>
            </div>
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              {[1,2,3].map((p) => (
                <div key={p} className="aspect-[3/4] rounded-xl bg-gradient-to-br from-[#F5F5F5] to-[#F4F2EC]" />
              ))}
            </div>
            ...
            {c.status === "Pending" && (
              <div className="mt-5 flex gap-2">
                ...
                <button className="px-4 py-2.5 rounded-xl bg-[#000000] text-white text-xs font-medium">Approve & reply</button>
              </div>
            )}
          </SoftCard>
        ))}
...
function Mini({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#FAFAF6] rounded-xl py-3">
      ...
      <div className="mt-0.5 text-sm font-medium text-[#000000]">{value}</div>
    </div>
  );
}
