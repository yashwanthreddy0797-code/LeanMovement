...
export function KPICard({
  ...
  const toneClass =
    tone === "up" ? "text-[#E11D2A] bg-[#FEE2E2]" :
    tone === "down" ? "text-[#E11D2A] bg-[#FEE2E2]" :
    "text-[#737373] bg-[#F5F5F5]";
  return (
    <div className="card-soft p-5">
      <div className="text-[11px] uppercase tracking-[0.18em] text-[#737373]">{label}</div>
      <div className="mt-3 flex items-baseline gap-1.5">
        <div className="text-3xl font-serif text-[#000000]">{value}</div>
        {unit && <div className="text-xs text-[#737373] font-medium">{unit}</div>}
      </div>
      {delta && (
...
export function SectionTitle({ eyebrow, title, action }: { eyebrow?: string; title: string; action?: ReactNode }) {
  return (
    <div className="flex items-end justify-between mb-5">
      <div>
        {eyebrow && <div className="text-[11px] uppercase tracking-[0.2em] text-[#737373] mb-1.5">{eyebrow}</div>}
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width="100" height="100" viewBox="0 0 100 100" className="-rotate-90">
        <circle cx="50" cy="50" r={r} stroke="#F5F5F5" strokeWidth="8" fill="none" />
        <circle cx="50" cy="50" r={r} stroke="var(--accent)" strokeWidth="8" fill="none"
          strokeDasharray={c} strokeDashoffset={off} strokeLinecap="round"
          style={{ transition: "stroke-dashoffset 0.8s ease" }} />
      </svg>
      <div className="-mt-[68px] text-center">
        <div className="text-lg font-serif">{value}<span className="text-[10px] text-[#737373] ml-0.5">{unit}</span></div>
        <div className="text-[10px] text-[#737373]">of {target}{unit}</div>
      </div>
      <div className="mt-[40px] text-xs text-[#404040] font-medium">{label}</div>
    </div>
  );
}
